...
questions =[
    ...
    "What's your preference in Transmission of car ?",
]
...
def output():
    ...
    df = pd.read_excel('cardatabase.xlsx', engine='openpyxl')
    
    car = df.loc[(df['Price']<=a1) & (df['Seat']==a2) & (df['Fuel']==a3) & (df['Car Type']==a4) & (df['Transmission']==a5)]
    car = car.values.tolist()
    if len(car) < 1:
        car = [['No Match Found', 'N/a', 'N/a', 'N/a', 'N/a', 'N/a', 'N/a']]
    dataToPrintLeft = f"Name : {car[0][0]} ({car[0][4]}) \nSeats : {car[0][2]} \nFuel Type : {car[0][3]} \nPrice : {car[0][1]} \nColor : {car[0][5]} \nTransmission : {car[0][6]}"
    ...
    try:
        folderLocation = r'CarImage\{} {}'.format(car[0][0],car[0][5])

        carImageList = []
        for path, subdirs, files in os.walk(folderLocation):
                for name in files:
                    carImageList.append(name)

        def imageCycle(image):
            img = Image.open(r'{}\{}'.format(folderLocation,image))
            img.thumbnail((600, 600), Image.ANTIALIAS)
            photo = ImageTk.PhotoImage(img)
            return photo

        photos = cycle(map(imageCycle, carImageList))

        def slideShow():
            img = next(photos)
            displayCanvas.config(image=img)
        
        displayCanvas = Label(root)
        slideShow()
        displayCanvas.pack()

        Btn = Button(root, text="Next Image", command = slideShow)
        Btn.pack()
    except:
        pass
